...
- (void)viewDidLoad
{
    ...
    NSString *count =  [[NSUserDefaults standardUserDefaults] objectForKey:@"seletePicCount"];
    NSInteger seleteCount = [count integerValue];
    ...
    selectNum = [[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"0/%ld",seleteCount?seleteCount:9] style:UIBarButtonItemStylePlain target:nil action:nil];
	...
	selectNum.title = [NSString stringWithFormat:@"%ld/%ld", (unsigned long)ASSETHELPER.selectdPhotos.count,seleteCount];
}
...
- (void)changeCount:(NSNotification *)notifi{
    ...
    selectNum.title = [NSString stringWithFormat:@"%ld/%ld", (unsigned long)ASSETHELPER.selectdPhotos.count,seleteCount?seleteCount:9];
