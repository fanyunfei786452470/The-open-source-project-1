//
//  YRTAccountSafeController.m
//  YRYZ
//
//  Created by Sean on 16/9/22.
...
@interface YRTAccountSafeController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong) YRUserInfoLoginModel *phoneModel;

@property (nonatomic,strong) YRAlertView * myAlertView;

@end

@implementation YRTAccountSafeController
...
- (void)loadData{
    ...
    [YRHttpRequest AccountInformationByInformationName:kLoginMethod success:^(NSDictionary *data) {
        ...
        self.phoneModel = self.infoLogin.lastObject;
...
- (void)setAlertView{
    ...
    self.myAlertView = [[YRAlertView alloc]initWithTitle:@"为了您的账户安全,请先设置支付密码" cancelButtonText:@"设置"];
    @weakify(self);
    self.myAlertView.addCancelAction = ^{
        @strongify(self);
...
- (void)processingImages{
    ...
    if (![WXApi isWXAppInstalled]) {
        ...
        for (YRUserInfoLoginModel *model in self.infoLogin) {
            if ([model.type isEqualToString:@"1"]&&[self.loginType integerValue]!=1) {
                [self.infoLogin removeObject:model];
                  break;
            }
        }
    }
    ...
    if (1) {
        ...
        for (YRUserInfoLoginModel *model in self.infoLogin) {
            if ([model.type isEqualToString:@"3"]&&[self.loginType integerValue]!=3) {
                [self.infoLogin removeObject:model];
                  break;
            }
        }
    ...
    if (![WeiboSDK isWeiboAppInstalled]) {
        ...
        for (YRUserInfoLoginModel *model in self.infoLogin) {
            if ([model.type isEqualToString:@"2"]&&[self.loginType integerValue]!=2) {
                [self.infoLogin removeObject:model];
                break;
            }
        }
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (section == 0) {
        ...
        if(self.imageArray.count==0)  return 1;
...
//            [accoutcell.headImageView setImageWithURL:[NSURL URLWithString:self.model.custImg] placeholder:[UIImage defaultImage]];
        ...
        if (indexPath.row == 0) {
            ...
            cell.textLabel.text = @"登录密码";
              cell.textLabel.font = [UIFont titleFont17];
            if (self.phoneModel.openId.length<10) {
            ...
            if (self.isHavePassword) {
                cell.detailTextLabel.text = @"更改";
                cell.detailTextLabel.font = [UIFont titleFont17];
                cell.detailTextLabel.textColor = RGB_COLOR(158, 158, 158);
            }else{
                cell.detailTextLabel.text = @"设置";
                  cell.detailTextLabel.font = [UIFont titleFont17];
...
- (void)setCellUIWithIndexPath:(NSIndexPath *)indexPath tableCell:(YRAccoutSafeCell *)cell{
    ...
    if (model.openId){
        ...
        if ([model.type integerValue]==4) {
            ...
            [cell.bageBtn setTitleColor:RGB_COLOR(102, 102, 102) forState:UIControlStateNormal];
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([YRUserInfoManager manager].currentUser){
        if (indexPath.section == 2) {
            if (indexPath.row == 1) {
                YRChangePayPassWordController *payVc = [[YRChangePayPassWordController alloc]init];
//                  YRChangeOneController  *payVc = [[YRChangeOneController alloc]init];
                if (self.isHavePassword) {
                    payVc.title = @"修改支付密码";
                }else{
                    payVc.title = @"设置支付密码";
                ...
                if (self.phoneModel.openId.length<10) {
                    YRAlertView *alertView = [[YRAlertView alloc] initWithTitle:@"先绑定手机号" cancelButtonText:@"取消" confirmButtonText:@"确定"];
