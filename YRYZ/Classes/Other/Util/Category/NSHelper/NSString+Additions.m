...
+(NSString *)HTTPBodyWithParameters:(NSArray *)parameters keyStr:(NSString*)keyStr
{
    ...
    [parameters enumerateObjectsUsingBlock:^(NSString *value, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([value isKindOfClass:[NSString class]]) {
            [parametersArray addObject:[NSString stringWithFormat:@"%@=%@",keyStr,value]];
        }
    ...
    return [[parametersArray componentsJoinedByString:@"&"] substringFromIndex:length];
}
...
- (BOOL)isMobileNumber
{
//    NSString *phoneRegex = @"+86|86)?(13[0-9]|14[57]|15[0-3]|15[5-9]|17[01678]|18[0-9])\\d{8}(,)?";
