...
@interface YRNewVideoDetailViewController ()
...
@implementation YRNewVideoDetailViewController
#pragma mark 初始化
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    NSString *count  = [NSString stringWithFormat:@"%ld",[self.productDetail.readCount ? self.productDetail.readCount : 0 integerValue]];
    ...
    NSMutableAttributedString *lstr22 = [[NSMutableAttributedString alloc]initWithString:self.forwardCount ? self.forwardCount :@""  attributes:@{NSForegroundColorAttributeName:[UIColor themeColor]}];
    ...
    NSInteger bonud = self.productDetail.transferBonud ? self.productDetail.transferBonud : 0;
    NSMutableAttributedString *lstr32 = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"%.2f",bonud*0.01] attributes:@{NSForegroundColorAttributeName:[UIColor themeColor]}];
...
- (void)infoReadStatus{
    ...
    [YRHttpRequest infoReadStatus:kInfoTypeVideo pid:self.productId success:^(id data) {
...
//    [self.nameLabel  addTapGesturesTarget:self selector:@selector(userImageClick)];
    ...
    [self.collectionButton setBackgroundImage:[UIImage imageNamed:@"yr_myCollect_no"] forState:UIControlStateNormal];
    self.collectionButton.frame = CGRectMake(SCREEN_WIDTH - 20 - 10, 5, 23, 23);
...
- (void)initContentView{
    ...
    self.titleLabel.text = self.productDetail.desc;
    ...
    videoView.mj_x = 10;
    videoView.mj_y = self.titleLabel.mj_y + self.titleLabel.mj_h;
    videoView.mj_w = SCREEN_WIDTH - 20;
    videoView.mj_h = 225;
    [videoView setImageWithURL:[NSURL URLWithString:self.productDetail.urlThumbnail] placeholder:[UIImage defaultImage] options:kNilOptions completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
...
- (void)playBtnClick:(UIButton *)sender{
    ...
    if ((self.productDetail.forwardStatus==1)&&(self.productDetail.readStatus==0)) {
        [self infoReadStatus];
...
- (void)initBottomView{
    ...
    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx == 0) {
            ...
            self.tranBottomButton .titleLabel.font = [UIFont systemFontOfSize:14];
...
- (void)userImageClick{
    ...
    [self pushUserInfoViewController:self.productDetail.custId withIsFriend:NO];
}
                ...
                if (tran) {
                    ...
                    self.rewardGiftView.rewardBlock = ^(RewardGiftModel *giftModel ,float money){
                        ...
                        if (self.smallNopass && [allMoneyStr floatValue] * 0.01 <= 50) {
                            ...
                            [YRHttpRequest sendReward:giftModel.giftid touserid:self.productDetail.custId type:2 infoId:self.productDetail.uid giftNum:1 password:@"" infoType:kInfoTypeVideo infoTitle:self.productDetail.desc ? self.productDetail.desc : @"" pid:@"" success:^(id data) {
...
- (void)sendReward:(NSString*)allMoneyStr giftModel:(RewardGiftModel*)giftModel touserid:(NSString*)touserid pid:(NSString*)pid{
    ...
    [YRPaymentPasswordView showPaymentViewWithMoney:allMoneyStr  paymentBlock:^(NSString  *password){
        ...
        [YRHttpRequest sendReward:giftModel.giftid touserid:touserid type:2 infoId:pid giftNum:1 password:password infoType:kInfoTypeVideo infoTitle:self.productDetail.infoTitle ? self.productDetail.desc : @"" pid:@"" success:^(id data) {
            [MBProgressHUD showSuccess:@"打赏成功"];
