...
@interface YRCircleFriendEarningView ()
...
@end
@implementation YRCircleFriendEarningView
- (id)init{
    ...
    if (self) {
        ...
        _backView = ({
            UIView * view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, VIEW_W(_bgView), VIEW_H(_bgView))];
            [view addSubview:_adImageView];
            [view sendSubviewToBack:_adImageView];
            view;
        });
        ...
        _totalNum = ({
            UIView * totalNum = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_headImage.frame)+10*SCREEN_H_POINT, CGRectGetMinY(_headImage.frame)+(_headImage.frame.size.height -40*SCREEN_H_POINT)/2, _titleBgView.frame.size.width - CGRectGetMaxX(_headImage.frame)-20*SCREEN_H_POINT, 40*SCREEN_H_POINT)];
            totalNum;
        });
        [_titleBgView addSubview:_totalNum];
        
        _numLabel = ({
            UILabel * numLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, _totalNum.frame.size.width/2, 40*SCREEN_H_POINT)];
        ...
        _num = ({
            UILabel * num = [[UILabel alloc]initWithFrame:CGRectMake(_totalNum.frame.size.width/2, 0, _totalNum.frame.size.width/2, 40*SCREEN_H_POINT)];
        ...
        _totalEarn = ({
            ...
            totalEarn.backgroundColor = [UIColor yellowColor];
            totalEarn;
        });
       // [_titleBgView addSubview:_totalEarn];
        ...
        _earnLabel = ({
            ...
            earnLabel.text = @"获得总收益";
            earnLabel.backgroundColor = [UIColor blackColor];
        ...
        _earn = ({
            ...
            earn.backgroundColor = [UIColor greenColor];
...
- (void)followEarnings:(UIButton *)sender{
    if (self.delegate) {
        ...
        [self.delegate followRransmitcircle:_circleModel];
    }
}
