...
+(void)refreshData:(NSString*)name{
    UIImage *image = [redPaperView createRedPaperImage:name];
    redPaperView.redPaperBgView.image = image;
}
...
+(void)showRedPaperViewWithName:(NSString*)name OpenBlock:(void(^)())openblock
{
    ...
    //[[YRRedPaperView sharedredPaperView] setupView:name];
}
...
-(void)setupView
{
    ...
    _redPaperBgView.bounds = YRRectMake(0 , 0 ,288, 353.5 - 30);
    //UIImage *image = [self createRedPaperImage:name];
...
-(UIImage *)createRedPaperImage:(NSString*)name
{
    ...
    [redPaperBgImage drawInRect:YRRectMake(0, 0, 288, 353.5 - 30)];
...
//    _redPaperBgView.image = [self createRedPaperImage:na];
}
...
-(void)showRedPaperView
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        [UIView animateWithDuration:0.05 animations:^{
            _redPaperBgView.bounds = YRRectMake(0 , 0 ,288 * 0.98,(353.5 - 30)* 0.98);
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.05 animations:^{
                _redPaperBgView.bounds = YRRectMake(0 , 0 ,288, 353.5 - 30);
            } completion:^(BOOL finished) {
