...
@property (nonatomic,weak) UILabel *groupCountLab;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        if (SCREEN_WIDTH == 414.f) {
            accessView.mj_x = SCREEN_WIDTH-30;
        }else{
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    NSInteger count = self.dataSource.count >18?18:self.dataSource.count;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (count>0) {
        ...
        DLog(@"displayName:%@,avatar:%@",item.displayName,item.avatarUrl);
        avatar = item.avatar;
        ...
        if (!avatar) {
            avatar = [UIImage imageNamed:@"yr_user_defaut"];
        }
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == count+1) {
        ...
        if ([self.delegate respondsToSelector:@selector(didSeleteLookPeopleDetailWithCustId:)]) {
            YWTribeMember *tribeMember = [self.dataSource objectAtIndexCheck:indexPath.row];
            [self.delegate didSeleteLookPeopleDetailWithCustId:tribeMember.personId];
        }
    }
}
/**
